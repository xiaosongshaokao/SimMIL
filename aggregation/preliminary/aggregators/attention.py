...
class AttentionPooling(nn.Module):
    ...
    def forward(self, feats):
        ...
        A = A.view(batch_size, bag_size, self.head).softmax(dim=1)    # (batch_size, bag_size, head)
        return torch.bmm(A.permute(0,2,1), feats).view(batch_size, in_feat) # (batch_size, head, bag_size) x (batch_size, bag_size, in_feat)
