...
from datasets.std_bag_dataset import StdBagDataset
from datasets.count_bag_dataset import CountBagDataset
from datasets.co_bag_dataset import CoBagDataset
from .base_runner import BaseRunner
class AssumptionRunner(BaseRunner):
    def build_data_loaders(self):
        self.logger.info("Building datasets")
        if self.args.assumption=="std":
            train_dataset = StdBagDataset(dataset=MyImageFolder(os.path.join(self.args.data, self.args.train_subdir),
                                    transform=self.train_augmentation),
                                        label_file=self.args.train_label_file,
                                        pos_target=self.args.pos_targets)
            
            val_dataset =  StdBagDataset(dataset=MyImageFolder(os.path.join(self.args.data, self.args.eval_subdir),
                                        transform=self.val_augmentation),
                                        label_file=self.args.val_label_file,
                                        pos_target=self.args.pos_targets)
        elif self.args.assumption=="count":
            train_dataset = CountBagDataset(dataset=MyImageFolder(os.path.join(self.args.data, self.args.train_subdir),
                                    transform=self.train_augmentation),
                                        label_file=self.args.train_label_file,
                                        pos_target=self.args.pos_targets,
            ...
            val_dataset =  CountBagDataset(dataset=MyImageFolder(os.path.join(self.args.data, self.args.eval_subdir),
                                        transform=self.val_augmentation),
                                        label_file=self.args.val_label_file,
                                        pos_target=self.args.pos_targets,
                                        pos_ratio=self.args.pos_ratio,
            ...
            train_dataset = CoBagDataset(dataset=MyImageFolder(os.path.join(self.args.data, self.args.train_subdir),
                                    transform=self.train_augmentation),
                                        label_file=self.args.train_label_file,
                                        pos_target=self.args.pos_targets)
            
            val_dataset =  CoBagDataset(dataset=MyImageFolder(os.path.join(self.args.data, self.args.eval_subdir),
                                        transform=self.val_augmentation),
                                        label_file=self.args.val_label_file,
                                        pos_target=self.args.pos_targets)
        else:
