...
import aggregators
from core.bag_mil_model import BagMILModel
...
class BaseRunner(object):
    ...
    def build_data_loaders(self):
        ...
        if self.args.debug:
            ...
            train_dataset = BagDataset(dataset=DebugDataset(os.path.join(self.args.data, self.args.train_subdir),
                                    transform=self.train_augmentation),
                                      label_file=self.args.train_label_file)
            
            val_dataset =  BagDataset(dataset=DebugDataset(os.path.join(self.args.data, self.args.eval_subdir),
                                     transform=self.val_augmentation),
                                      label_file=self.args.val_label_file)
        else:
            train_dataset = BagDataset(dataset=MyImageFolder(os.path.join(self.args.data, self.args.train_subdir),
                                    transform=self.train_augmentation),
                                      label_file=self.args.train_label_file)
            
            val_dataset =  BagDataset(dataset=MyImageFolder(os.path.join(self.args.data, self.args.eval_subdir),
                                     transform=self.val_augmentation),
                                      label_file=self.args.val_label_file)
    ...
    def build_model(self):
        ...
        model.backbone.fc.requires_grad = True
        return model
    ...
    def load_pretrained(self, model):
        try:
            ...
            if self.args.pretrained_type in ['moco']:
                ...
                for k in key_list:
                    # self.logger.info("Transforming {} to {}".format(k, k.replace(k.split(".",1)[0], 'backbone')))
                    state_dict["{}".format(k.replace(k.split(".",1)[0], 'backbone'))] = state_dict.pop("{}".format(k))
                ...
                def load_model(model, state_dict, strict=False):
                    ...
                    key_list = list(filter(lambda x: not x.startswith('backbone.'), state_dict.keys()))
                    for k in key_list:
                        self.logger.info("Transforming {} to backbone.{}".format(k, k))
                        state_dict["backbone.{}".format(k)] = state_dict.pop(k)
                    msg = model.load_state_dict(state_dict, strict=strict)
    ...
    def run(self):
        ...
        self.validate(val_loader, best_model, criterion)
    ...
    def validate(self, val_loader, model, criterion):
        ...
        self.logger.info(' * BAUC {:.5f}'.format(auc))
