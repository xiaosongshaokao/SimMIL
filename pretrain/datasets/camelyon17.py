...
if __name__=="__main__":
    class_label_dict = {"itc_nontumor": 0, 
                        "itc_tumor": 1, 
                        "macro_nontumor":2, 
                        "macro_tumor":3, 
                        "micro_nontumor":4, 
    ...
    partial_class = "neg_nolabel"
    
    root = "/remote-home/source/DATA/CAMELYON17/DATA/train/256_20X/"
    data = CameLyon17(root, cls_label_dict=class_label_dict, partial_class=[partial_class])
    print("Total instances: {}\n".format(len(data)))

    for k, v in class_label_dict.items():
        ins_num = np.array(data.instance_labels)
        ins_num = (ins_num==v).sum()
        print("For folder {}, instance number: {}, ratio: {} ".format(k, ins_num, ins_num/len(data)))
