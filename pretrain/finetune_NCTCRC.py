...
def build_dataset(augmentation, mode='train'):
    ...
    postfix = 'train_ins.txt' if mode=='train' else 'val_ins.txt'
...
def eval_linear(args):
    ...
    # otherwise, we check if the architecture is in torchvision models
        ...
        model = torchvision_models.__dict__[args.arch](pretrained=args.imagenet_pretrain)
        embed_dim = model.fc.weight.shape[1]
        model.fc = LinearClassifier(embed_dim, num_labels=args.num_labels)
    else:
    ...
    if args.evaluate:
        utils.load_pretrained_weights(model, args.arch, args.patch_size)
...
def validate_network(val_loader, model, n, avgpool):
    ...
    metric_logger = utils.MetricLogger(delimiter="  ")
    header = 'Test:'
    for inp, target in metric_logger.log_every(val_loader, 40, header):
        # move to gpu
        inp = inp.cuda(non_blocking=True)
        target = target.cuda(non_blocking=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--imagenet_pretrain', default=False, type=bool, help='use imagenet pretrain or not')
    ...
    if not os.path.exists(args.output_dir):
        ...
        print('make dir {} success'.format(args.output_dir))
    eval_linear(args)
